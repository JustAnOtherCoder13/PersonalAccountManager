...
class MyAccountDetailViewModel @Inject constructor(
    ...
    fun onStart(selectedAccountId: String) {
        ...
        viewModelScope.launchOnIOCatchingError(
            block = {
                GetAccountAndRelatedOperationsForAccountIdInteractor.getAccountForIdWithRelatedOperationsAsFlow(
                    id,
                    this
                )
                        ...
                        dispatchAction(
                            AppActions.MyAccountDetailScreenAction.UpdateAccountAndMonthlyOperations(
                                ...
                                relatedMonthlyOperations = accountWithRelatedOperations.relatedOperations.filter {
                                    isEmitDateMonthIsActualMonth(it)
                                }
                            )
                        )
                    }
    ...
    private fun isEmitDateMonthIsActualMonth(it: OperationUiModel) =
        it.emitDate.getDateMonth().compareTo(Calendar.getInstance().time.getDateMonth()) == 0
